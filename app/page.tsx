...
import {
  ...
  WordReveal,
  SimpleOpacityReveal,
} from "./components/RevealElements";
  ...
  useGSAP(
    () => {
      ...
      if (quoteRef2.current) {
        const words2 = gsap.utils.toArray(
          quoteRef2.current.querySelectorAll(".word"),
        ) as HTMLElement[];
        gsap.fromTo(
          words2,
          { opacity: 0.1 },
      ...
      if (quoteRef3.current) {
        const words3 = gsap.utils.toArray(
          quoteRef3.current.querySelectorAll(".word"),
        ) as HTMLElement[];
        gsap.fromTo(
          words3,
          { opacity: 0.1 },
      ...
      if (quoteRef4.current) {
        const words4 = gsap.utils.toArray(
          quoteRef4.current.querySelectorAll(".word"),
        ) as HTMLElement[];
        gsap.fromTo(
          words4,
          { opacity: 0.1 },
        ...
        <div className="mt-0 lg:mt-24 px-8">
          <div className="mb-8 flex flex-col lg:gap-4 justify-between">
            <div>
              <h2
                ref={quoteRef}
                className="relative z-90 text-3xl md:text-7xl text-left font-bold text-gray-800 font-jakarta max-w-[30ch] flex flex-wrap gap-x-2 md:gap-x-4 gap-y-1 md:gap-y-2"
              >
                {t("home.quote1")
                  .split(" ")
                  .map((word, index) => (
          ...
          <SimpleOpacityReveal className="text-lg md:text-2xl text-gray-600 font-medium mb-8">
            {t("home.quote1Desc")}
          </SimpleOpacityReveal>
        </div>
      </section>
          ...
          <div className="flex flex-col">
            ...
            <div className="w-full flex justify-end md:block hidden">
              <div className="flex flex-col items-end mb-12 relative absolute -bottom-10 right-0">
                ...
                <WordReveal
                  text={t("home.quote2Annotation")}
                  className="font-caveat leading-snug text-xl md:text-3xl max-w-lg text-center justify-center uppercase leading-snug font-semibold text-gray-800"
                />
              </div>
            </div>

            <SimpleOpacityReveal className="text-lg md:text-2xl text-gray-600 font-medium md:mt-12">
              {t("home.quote2Desc")}
            </SimpleOpacityReveal>
          </div>
